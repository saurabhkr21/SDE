...
export default function Header({ handleOpen, selectCategory, setSelectCategory, searchQuery, setSearchQuery }) {
    ...
    return (
        <header className='flex flex-col  w-full h-auto bg-slate-900 '>
            <nav className='flex flex-col sm:flex-row justify-between items-center gap-3 p-4'>
                <span className='flex items-center'>
                    <h1 className='text-violet-400 italic font-bold text-3xl'>
                        My Memes Box</h1>
                </span>
                <span className='flex items-center gap-1'>
                    <form className='gap-2 flex items-center'>
                        <input type="search"
                            ...
                            className='text-white w-[110px] sm:w-auto border-amber-50 border-1 rounded-xl p-1 focus:border-violet-900' placeholder='search...' />
                    ...
                    <button className='text-white  bg-pink-700 rounded-2xl  px-2 pb-1 ' onClick={handleOpen}>
                        <span className='px-1 text-xl text-gray-500 font-extrabold py-2'>+</span>Memes
                    </button>
                </span>
            </nav>
